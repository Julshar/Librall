...
unsigned int Random::genValue(unsigned int min, unsigned int max)
{
  ...
  /*
  * WORTH TRYING - std::uniform_int_distribution
  *
  * this function does math to generate number
  * that will be uniformly distributed.
  * It means that each number from given range
  * has exactly the same chance to be generated.
  * 
  * But this math is done below with standard operations.
  */
